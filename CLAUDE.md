...
This is an **infrastructure load-testing project**, not a production trading system. The end goal: collect real historical market data from Upbit, feed it to a "trader" application that replays it as massive simulated trading traffic against an order-acceptance API + matching engine, and use that traffic to load-test the backend + infra. Four independent Go modules live in this repo so far: `backend/` (market data collection + serving), `trader/` (data-replay + bot engine), `orderapi/` (order acceptance, role "A" — see below), and eventually the matching engine (role "B", not built yet). `frontend/` is a separate Vue/Vite app (see its own `frontend/README.md`) not yet wired to anything backend-side.

Per `requirements.md` 1.2.4, **role A (order acceptance) and role B (matching engine) are architecturally separate applications that only ever communicate via Kafka** (A publishes to the `orders` topic, B would consume it and publish to `executions`) — never direct calls. `orderapi/` is role A; whoever builds the matching engine builds it as yet another separate module/app, not by extending `orderapi/`.
...
`backend/`, `trader/`, and `orderapi/` are **separate Go modules** (each has its own `go.mod` — `module backend`, `module trader`, `module orderapi`), so `cd` into whichever one you're working on before running Go commands; there is no shared workspace file tying them together.
...
go test ./...
```
Env vars (`.env` locally via `godotenv`, gitignored): `BACKEND_URL` (default `http://localhost:8080`), `ORDERAPI_URL` (default `http://localhost:8081`).
...
go test ./...
```
Env vars (`.env` locally via `godotenv`, gitignored): `PORT` (default `8081`), `KAFKA_BROKER` (default `localhost:9092`), `ORDERS_TOPIC` (default `orders`).

### Tests

All three modules have `_test.go` files now (table-driven, stdlib `testing` only — no external test/mock libraries). Convention: tests live beside the code they cover, same package (white-box, so unexported helpers like `order.tickTier` are reachable), `TestXxx` names, `t.Run` subtests for table cases. `go test` (and everything else that isn't `go build`/`go run`) automatically excludes `_test.go` files from the compiled binary — that's a Go toolchain guarantee based on the filename suffix, not something this project configures.

What's covered and why: pure/deterministic logic that's easy to get subtly wrong — `order.RoundToTick` (tick-size table + rounding), `bot.MarketState`'s circular buffer (wraparound indexing), `MarketMakerBot.Decide`, `dataset.BuildBatch`/`BuildStream` (Upbit field → normalized field mapping, event merge+sort), `dataset.localStorage` save/load round-trip, and `backend`'s `manifestHandler`/`fileHandler` (via an in-memory fake `dataset.Storage` in `server_test.go`, and — since `fileHandler`'s `r.PathValue()` only populates when matched through a real `http.ServeMux`, not on a bare handler call — a small mux built in the test with the same route pattern as `main.go`). `orderapi` follows the same handler-test pattern: `server_test.go`'s `fakePublisher` (implements the `Publisher` interface) stands in for Kafka, so `acceptOrderHandler`/`cancelOrderHandler` are tested (missing key, invalid market, success, idempotent retry doesn't re-publish, publish failure isn't cached so a retry can succeed, cancel-not-found, cancel-then-idempotent-no-op, cancel-already-filled) without a real broker; `validate_test.go`/`idempotency_test.go`/`order_test.go` cover the pure logic pieces the same way `backend`/`trader` do.
...
Brings up a single-node KRaft-mode Kafka broker (`apache/kafka:3.7.0`) on `localhost:9092`, auto-creating topics (`KAFKA_AUTO_CREATE_TOPICS_ENABLE=true`). `orderapi/.env` holds `KAFKA_BROKER` for connecting to it (gitignored — copy/create locally, it's not committed) — `orderapi` is the only thing in this repo that publishes to Kafka (`backend` never did in practice, and its unused `kafka/producer.go` wrapper has since been deleted — see "Data flow" below).

**Client-side gotcha**: the broker allowing auto-created topics is not sufficient by itself — `segmentio/kafka-go`'s `kafka.Writer.AllowAutoTopicCreation` defaults to `false`, so a client that doesn't explicitly set it to `true` gets `[3] Unknown Topic Or Partition` against a brand-new topic even though the broker would happily create it. `orderapi/kafkaclient.go` sets it explicitly (see Architecture below). Also note topic creation is asynchronous on the broker: the very first publish to a never-seen topic can still fail once even with the flag set — a retry (same or a different request) succeeds. This is part of why the idempotency store never caches 5xx responses (see `orderapi/idempotency.go` below): it's what lets that retry actually work.
...
- **`orderapi/`** — separate Go module (`module orderapi`, own `go.mod`), the order-acceptance API: role **A** per `requirements.md` 1.2.4. Implements `docs/api-specification.md`'s contract. Architecturally standalone from the (not-yet-built) matching engine (role **B**) — the two only ever talk via Kafka (`orders` topic in, `executions` topic out, once B exists), never a direct call; `orderapi` doesn't import anything from `backend`/`trader` and re-declares its own copies of small shared tables (20-market list, tick-size tiers) rather than sharing Go types across module boundaries, same rationale as `backend`↔`trader`.
  - **`main.go`** — `LoadConfig()`, `NewOrderStore()`, `NewIdempotencyStore()`, `NewOrderProducer(broker, topic)` (deferred `Close()`), registers `POST /v1/orders` and `DELETE /v1/orders/{orderId}` on a stdlib `ServeMux`, `ListenAndServe`.
  - **`config.go`** — `LoadConfig()` reads `.env` via `godotenv` (same pattern as `backend/config`), env vars listed in Commands above.
  - **`order.go`** — `Order` (status one of `ACCEPTED`/`PARTIALLY_FILLED`/`FILLED`/`CANCELED`) and `OrderStore` — an in-memory, mutex-protected map keyed by `orderId`, tracking accepted orders purely so `DELETE` has something to look up/cancel (no persistence; this is not the S3-backed order-*record* store, that's `trader/orderstore` — different concern, different module). `NewOrderID(now)` generates `ord_{YYYYMMDD}_{7-digit seq}` from an atomic in-memory counter.
  - **`validate.go`** — independently declares `TargetMarkets` (20 markets) and a tick-size tier table, used by `isTargetMarket`/`validateSide`/`validatePrice`/`validateQuantity`. Deliberately the *opposite* policy from `trader/order/ticksize.go`'s `RoundToTick`: here a price that isn't an exact multiple of its tier's tick size is **rejected** (`INVALID_PRICE_UNIT`), not silently rounded — the API is the enforcement boundary, rounding belongs upstream in whatever generates the order.
  - **`idempotency.go`** — `IdempotencyStore`, an in-memory `map[key]cachedResponse{status, body}`. `Put` does **not** overwrite an existing entry — first response for a given `Idempotency-Key` wins and is replayed verbatim on retry, including for cached 4xx validation failures. Deliberately caches success and deterministic 4xx, but **never** a 5xx (Kafka publish failure) — a transient failure must stay retryable under the same key, which the "auto-topic-creation's first publish can fail once" behavior (see Local Kafka above) depends on.
  - **`kafkaclient.go`** — `Publisher` interface (`PublishNew`/`PublishCancel`) exists so `server.go`'s handlers can be tested with a fake, without a real broker (`server_test.go`'s `fakePublisher`). `OrderProducer` is the real implementation (`segmentio/kafka-go`), keyed by `market` and using `&kafka.Hash{}` — chosen deliberately over `&kafka.LeastBytes{}` (which the repo's now-deleted `backend/kafka/producer.go` used, discovered while re-reading that file for reference before removing it): `LeastBytes` ignores the message `Key` entirely and picks partitions by least-accumulated-bytes, which would have broken the "same market always same partition" guarantee (FR-04/NFR-07). Also sets `AllowAutoTopicCreation: true` — see the Local Kafka gotcha above for why. The Kafka message shape (`{"type":"NEW"|"CANCEL","orderId","market","side","price","quantity","acceptedAt"}`) is **provisional** — `docs/api-specification.md` §6 defers to a separate Kafka message-spec doc (FR-22) that doesn't exist yet; reconcile with that doc once it's written.
  - **`server.go`** — `acceptOrderHandler` (`POST /v1/orders`): missing `Idempotency-Key` → 400; cached key → replay cached response verbatim (no re-validation, no re-publish); else validates market/side/price/quantity (each failure cached under the key); builds the `Order` with `AcceptedAt` in **UTC**, ms-precision (`nowISO()`) — `api-specification.md` §1.2 mandates UTC for this API specifically, an independent decision from the KST choice made for the market-data API/`backend`, since they're different docs/different owners; publishes via `Publisher.PublishNew` (failure → 500, logged, **not cached**, so the client can retry the same key); on success, saves to `OrderStore`, caches the 202 response, returns it. `cancelOrderHandler` (`DELETE /v1/orders/{orderId}`): 404 if unknown; 409 `ORDER_ALREADY_FILLED` if already `FILLED` (can't happen yet with no matching engine, but the check exists for when B starts updating status via `executions`); if already `CANCELED`, returns the previously-stored cancellation result unchanged without re-publishing (idempotent-by-construction, no idempotency-key needed since order state itself already encodes "already canceled"); otherwise marks `CANCELED`, publishes a `CANCEL` event on the same market key (so B, once it exists, sees NEW/CANCEL for a market in order), returns 200. A `TODO` marks NFR-13 (429 on consumer-lag) as unimplemented — there's no consumer (B) yet, so there's no lag to measure.
