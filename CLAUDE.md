...
Brings up a single-node KRaft-mode Kafka broker (`apache/kafka:3.7.0`) on `localhost:9092`, auto-creating topics (`KAFKA_AUTO_CREATE_TOPICS_ENABLE=true`). `backend/.env` holds `KAFKA_BROKER` for connecting to it (gitignored — copy/create locally, it's not committed); `orderapi/.env` needs the same variable and is what actually exercises this broker today (`orderapi` is the first thing in this repo that publishes to Kafka for real — `backend/kafka/producer.go` remains unused).
...
- **`orderapi/`** — separate Go module (`module orderapi`, own `go.mod`), the order-acceptance API: role **A** per `requirements.md` 1.2.4. Implements `docs/api-specification.md`'s contract. Architecturally standalone from the (not-yet-built) matching engine (role **B**) — the two only ever talk via Kafka (`orders` topic in, `executions` topic out, once B exists), never a direct call; `orderapi` doesn't import anything from `backend`/`trader` and re-declares its own copies of small shared tables (20-market list, tick-size tiers) rather than sharing Go types across module boundaries, same rationale as `backend`↔`trader`.
  ...
  - **`kafkaclient.go`** — `Publisher` interface (`PublishNew`/`PublishCancel`) exists so `server.go`'s handlers can be tested with a fake, without a real broker (`server_test.go`'s `fakePublisher`). `OrderProducer` is the real implementation (`segmentio/kafka-go`), keyed by `market` and using `&kafka.Hash{}` — **deliberately not** `backend/kafka/producer.go`'s `&kafka.LeastBytes{}`, which was found (while re-reading that file for reference) to ignore the message `Key` entirely and pick partitions by least-accumulated-bytes, silently breaking the "same market always same partition" guarantee (FR-04/NFR-07). That's a latent bug in the existing unused `backend/kafka/producer.go`, not fixed there since it's out of scope/unused; documented here so it isn't reintroduced if that code ever gets wired up. Also sets `AllowAutoTopicCreation: true` — see the Local Kafka gotcha above for why. The Kafka message shape (`{"type":"NEW"|"CANCEL","orderId","market","side","price","quantity","acceptedAt"}`) is **provisional** — `docs/api-specification.md` §6 defers to a separate Kafka message-spec doc (FR-22) that doesn't exist yet; reconcile with that doc once it's written.
