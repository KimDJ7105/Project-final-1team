...
- **`trader/`** — separate Go module (`module trader`, own `go.mod`), the data-replay + bot engine described in `docs/market-data-api.md`. `main.go` parses `-backend`/`-date`/`-speed` flags, fetches the manifest, builds a `map[string]*MarketState` (one per market, shared between that market's own replay goroutine and the global bots below), then spawns one goroutine per market (`replay.go`'s `ReplayMarket`) sharing a single `*http.Client` (`client.go`'s `NewHTTPClient`, 5-minute timeout to tolerate on-demand collection latency on a cache miss). Each goroutine fetches its batch+stream, then walks `stream.Events` in order, sleeping `(events[i].ts - events[i-1].ts) / speed` between events before logging each one and calling `state.Update(price)`. One market's error is logged and collected into a failure summary at the end; it does not stop or cancel the others (plain `sync.WaitGroup` — see `wg.Go` usage throughout `trader/`, Go 1.24+ — deliberately not `errgroup`, whose `WithContext` cancels sibling goroutines on the first error, exactly the propagation this design avoids). `trader/types.go` re-declares the JSON shapes backend returns (`Manifest`, `BatchFile`, `StreamFile`, ...) rather than importing `backend/dataset` — the two modules are meant to only ever talk over HTTP+JSON, never share Go types at compile time.
  - **`MarketState` (`bot.go`)**: not just a single latest price — a fixed-size circular buffer (`priceHistorySize`, currently 30) of recent prices behind a `sync.RWMutex`. `Price()` returns the most recent entry (fast path for the per-market algorithmic bots below); `History()` returns the buffer oldest→newest (for the AI bots, which need a price *series* to reason about momentum/mean-reversion, not just a point value).
  - **Per-market bots (FR-16, `Bot` interface)**: `marketBots()` — **`MarketMakerBot`/`NoiseBot`/`LargeOrderBot`** (`marketmaker.go`/`noise.go`/`largeorder.go`) — plain rule-based algorithms, one independent instance *per market* (60 total across 20 markets), run inside `ReplayMarket` as goroutines on a `time.Ticker` at `bot.Interval()/speed`, reading that market's own `MarketState`. Per-market is deliberate here: a market maker has to quote against one specific order book, it can't function "portfolio-wide". Bot goroutines share the market's replay lifetime (started alongside the event loop, cancelled via `context.WithCancel` when it ends).
  - **Global bots (FR-16, `GlobalBot` interface, `globalbot.go`)**: **`MomentumAIBot`/`MeanReversionAIBot`** (`ai_bot.go`) are **not** per-market — decided deliberately, both for cost (2 Bedrock calls per judgment cycle instead of 40) and because "AI trader scanning the whole board and picking where to act" fits the concept better than 20 independent copies. `GlobalBot.Decide(states map[string]*MarketState) []GlobalDecision` receives *every* market's `MarketState` in one call and can return decisions tagged with whichever markets it wants to act on. `RunGlobalBots` runs both as their own goroutines from `main.go` (not inside any single market's `ReplayMarket`), sharing the same `states` map main.go built — started once, stopped only when *all* markets finish replaying (`cancel()` after the per-market `wg.Wait()`, not per-market). Both `Decide` implementations currently always return `nil` — connection points for AWS Bedrock, marked with `TODO` comments, deliberately not implemented yet. **Known unresolved issue** (marked with `TODO` in `globalbot.go`): global bot tickers are scaled by `/speed` like the algorithmic bots, but real Bedrock call latency won't shrink just because replay is sped up — revisit when actually wiring Bedrock (either decouple the AI bots' cadence from `speed`, or skip a tick if the previous call is still in flight).
  ...
  - **Order submission**: `order.go`'s `OrderSubmitter` interface decouples "decided to trade" from "how the order actually gets sent" — `POST /v1/orders` (per `docs/api-specification.md`) readiness is unknown, so `main.go` currently injects `LogOnlySubmitter` (logs only). Swap in an `HTTPOrderSubmitter` behind the same interface once that endpoint is confirmed live; nothing else in `trader/` needs to change.
