...
- **`backend/main.go`** — entrypoint. Starts an HTTP server (`net/http`, stdlib `ServeMux`, no router dependency) on `cfg.Port` (env `PORT`, default `8080`). Routes: `POST /v1/collect`, `GET /v1/markets/data`. Does not currently call Kafka.
...
The trader app is **pull-based**: it does not receive data automatically at startup. It requests simulation data for a period from the backend, and the backend responds with the JSON file(s) covering that period (read from S3). The *collection-trigger* side of this is implemented (`POST /v1/collect`, see Architecture above). The *read/serve* side is partially implemented — see [docs/market-data-api.md](docs/market-data-api.md) for the full spec and sequence diagram; summary: a `GET /v1/markets/data?date=...` manifest endpoint (**implemented**) returns URLs for all 40 files (20 markets × batch/stream), the trader fetches them individually/in parallel via `GET /v1/markets/{market}/{batch|stream}` (**not yet implemented**) — backend proxies file bytes, no S3 presigned URLs, so the trader never needs S3 credentials — and per-market replay pacing (walking `events` by `ts` gaps) happens entirely client-side in memory, decoupled from the HTTP download.

**Decided**: the on-demand-generation trigger (hybrid design above) belongs to the per-file GET handler, not the manifest — the manifest stays cheap/fast (no existence checks, no `dataset.Storage` access at all) and only the not-yet-built `GET /v1/markets/{market}/{batch|stream}` checks whether its file exists and triggers `collectMarket` for just that one market if not. Implementation note for whoever builds that handler: `collectMarket` produces both batch and stream together in one call, so if a trader requests both for the same market at nearly the same time and both are missing, naively triggering collection from each request would run `collectMarket` twice concurrently for the same market+date — needs a per-(market, date) lock/dedup so the second request waits on or reuses the first's in-flight collection instead of re-running it.
