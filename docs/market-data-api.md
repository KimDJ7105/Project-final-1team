...
| `GET /v1/markets/{market}/{kind}` (개별 파일) | 트레이더 → 백엔드 | 설계 확정, 미구현 (다음 작업) |
...
### `GET /v1/markets/{market}/{kind}?date=2026-07-27` (개별 파일, 미구현 — 다음 작업)
...
하이브리드 생성 설계(CLAUDE.md 참고: 사전 생성 + 온디맨드 생성 모두 지원)에 따라, 요청받은 날짜의 파일이 아직 없을 수 있다. 이때 **누가 수집을 트리거하느냐**를 다음과 같이 정했다:

- **매니페스트는 트리거하지 않는다.** 존재 확인 자체를 안 하므로(위 참고) 항상 빠르게 응답한다.
- **개별 파일 GET(`/v1/markets/{market}/{kind}`)이 트리거한다.** 요청받은 파일이 없으면 그 자리에서 해당 마켓만 `collectMarket`으로 수집한 뒤 서빙한다.

**구현 시 주의할 점**: `collectMarket`은 batch와 stream을 한 번의 호출로 같이 만든다. 트레이더가 같은 마켓의 batch/stream을 거의 동시에 요청하면(매니페스트 받은 직후 병렬 다운로드 시작 — 4절 참고), 두 요청이 동시에 "파일 없음"을 보고 `collectMarket`을 중복 실행할 수 있다. **마켓+날짜 단위 락(또는 in-flight 요청 재사용)**으로 이 중복 실행을 막아야 한다.
