...
flowchart TB
    ...
    class KAFKA_MKT,KAFKA_ORD,KAFKA_EXEC stream
...
| Amazon MSK (Serverless) | 시세/orders/executions 토픽 | 컴포넌트 간 비동기 메시징 — 마켓명 기준 파티션으로 순서 보장. 세 EKS 클러스터 밖의 관리형 서비스라 매칭 엔진의 측정 자원을 갉아먹지 않는다. IAM 인증으로 IRSA 패턴을 그대로 재사용 |
...
**시세 처리**
1. 사용자가 트레이딩 기간을 선택하면, 시세 수집기가 업비트에서 해당 기간의 20개 마켓 시세를 수신한다
2. 수신한 시세는 Kafka 시세 토픽에 발행되고, 원본은 S3에 저장된다
3. AI 트레이더가 Kafka를 구독해 시세를 받는다(최초 일괄 + 실시간 순차)
