...
- **`TRADE_ORDER.remaining_quantity` 배관 추가(2026-08-06)** — 매칭 엔진이 별도로 발행하지 않고, "기록기"가 체결 반영 트랜잭션 안에서 `remaining_quantity = remaining_quantity - 체결수량`으로 스스로 계산한다(`recorder/store/postgres.go`의 `updateFill`, `UPDATE ... RETURNING`으로 잠금+계산+쓰기를 한 SQL 문에서 원자적으로 처리 — 여러 체결이 동시에 들어와도 갱신 레이스가 없음).
