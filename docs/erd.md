...
`orderapi`(role A)·`matching`(role B)·`trader`·`replayengine`·`recorder`를 실제로 구현·검증해보니, 이 ERD가 전제하는 정보 중 일부가 **지금의 API 계약/코드로는 채울 방법이 없다.** 테이블/컬럼 자체가 잘못됐다기보다, 그 값을 만들어 넘겨주는 배관이 시스템 어디에도 아직 없다는 뜻이다 — 그래서 "삭제"가 아니라 "지금 당장은 못 채우는 것"으로 정리해둔다. (2026-08-06 논의로 `TRADER_BOT`/`REPLAY_ENGINE_MARKET`/`MARKET.frequency_group`/`MATCHING_ENGINE_ASSIGNMENT`/`client_request_id`/`mode`/`remaining_quantity` 항목은 결론이 나서 4장으로 옮겼고, 2026-08-07 `REPLAY_SESSION`/`replay_session_id`도 "만들지 않기로", `source_order_id`도 "구현 완료"로 결론이 나서 4장으로 옮겼다 — 아래는 아직 결론이 안 난 항목만 남았다.)

- **FR-19(리플레이 엔진 분산 실행)와 "동시 실행 방지" 세션 가드의 충돌 — 방향은 결정됨(2026-08-07), 세션 가드 수정은 다음 작업.** `requirements.md` FR-19는 "단일 리플레이 엔진으로 부하가 부족할 경우 마켓 단위로 분산 실행한다"고 명시하는데, `orderapi/session/session.go`의 `Claim`은 `owner` 값과 무관하게 `orderapi:session:active`라는 단 하나의 Redis 키에 `SETNX`를 거는 구조라(`session.go:88`) 샤드 0이 `Claim`에 성공하면 샤드 1은 owner 이름이 뭐든 무조건 409(`ErrAlreadyActive`)로 거부되어 시작조차 못 한다. 즉 FR-19가 요구하는 "여러 `replayengine` 프로세스를 동시에 띄워서 분산 처리"하는 상황 자체가 지금 구조로는 일어날 수 없다.
  - **팀 결정 (1) — 점진적 스케일 테스트**: 목표치를 미리 정해놓고 한 번에 다 띄우는 대신, 인스턴스 **2대부터 시작해 4대→7대 순으로 늘려가며** 실제 병목이 어디서 나타나는지(`replayengine` 자체인지, `orderapi`/Kafka/`matching` 쪽인지) 관찰하기로 함. 참고 계산: 인스턴스 1대 실측 처리량(`-speed=500`, 초당 약 1,450건, `CLAUDE.md` 참고)과 NFR-01 목표(초당 10,000건)를 단순 비례로 역산하면 대략 7대가 근접값 — 마켓이 20개뿐이라 현재 설계(마켓 1개=고루틴 1개)로는 최대 20대까지만 의미가 있으므로 7대는 그 한계에 여유가 있다.
  - **팀 결정 (2) — `matching`의 FR-11 같은 동적 재분배 로직은 만들지 않기로 확정**. `replayengine`은 계속 떠있는 서비스가 아니라 `-date` 하나를 처리하고 끝나는 1회성 배치 Job(K8s `Job`으로 실행)이라, `-shard-count`/`-shard-index`는 실행 시작 시 한 번만 정해지면 되고 실행 도중 인스턴스가 늘고 줄 일이 없다 — FR-11이 필요했던 "계속 떠있는 서비스에서 인스턴스가 예측 불가능하게 죽고 다시 뜨는" 전제 자체가 `replayengine`에는 없다(마켓별 기록 파일이 서로 완전히 독립적이라 애초에 조율이 필요 없다는 점도 동일한 이유, `CLAUDE.md`의 `replayengine/main.go` 설명 참고).
  - **아직 안 풀린 것 — 다음 작업**: 세션 가드 자체를 어떻게 고칠지는 아직 설계되지 않았다. 같은 실행의 여러 샤드를 "하나의 실행"으로 취급해 가드를 통과시키는 구체적 메커니즘(예: 같은 실행의 샤드들이 세션 ID를 공유해 재진입 허용)을 다음에 설계·구현해야 한다.
