...
func TestInMemoryRecorderAccumulatesPerMarket(t *testing.T) {
	...
	r.Record(NewOrder("KRW-BTC", bot.Decision{Side: "BUY", Price: 100, Quantity: 1}))
	r.Record(NewOrder("KRW-BTC", bot.Decision{Side: "SELL", Price: 101, Quantity: 1}))
	r.Record(NewOrder("KRW-ETH", bot.Decision{Side: "BUY", Price: 50, Quantity: 1}))
...
func TestInMemoryRecorderSnapshotSortsByTS(t *testing.T) {
	...
	r.Record(Order{Market: "KRW-BTC", TS: 300, Side: "SELL"})
	r.Record(Order{Market: "KRW-BTC", TS: 100, Side: "BUY"})
	r.Record(Order{Market: "KRW-BTC", TS: 200, Side: "BUY"})
...
func (s stubSubmitter) Submit(context.Context, Order) error { return s.err }
...
func TestRecordingSubmitterRecordsOnlyOnSuccess(t *testing.T) {
	...
	ok := RecordingSubmitter{Next: stubSubmitter{err: nil}, Recorder: recorder}
	if err := ok.Submit(context.Background(), o); err != nil {
		t.Fatalf("성공 케이스에서 에러: %v", err)
	}
	if got := len(recorder.Snapshot("KRW-BTC")); got != 1 {
		t.Errorf("제출 성공 시 1건 기록돼야 하는데 %d건", got)
	}

	failing := errors.New("제출 실패")
	fail := RecordingSubmitter{Next: stubSubmitter{err: failing}, Recorder: recorder}
	if err := fail.Submit(context.Background(), o); !errors.Is(err, failing) {
